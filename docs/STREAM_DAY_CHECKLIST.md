...
| อันดับ | `01` กลาง-สูงสุด · `02` ซ้าย · `03` ขวา · เรียงยอดมาก → น้อย |
| ช่องที่ยังไม่มี | **เว้นว่าง** (ไม่มีชื่อ/ยอด/ควัน) และ 0 ผู้สนับสนุน = ข้าม Podium |
| ควัน | เอนจินเดิม (เหมือนการ์ด Alert/Last Follow) ย้อมสีต่ออันดับ: ที่ 1 ทอง · ที่ 2 เงิน · ที่ 3 ทองแดง (ไม่เขียนคำ Gold/Silver/Bronze บนจอ) |
| ตำแหน่ง/ขนาด | เท่ากับการ์ด Last Follow (`bottom-left` · กว้าง `calc(min(28vw,460px) * .82)` ≈ 380px) |

### พารามิเตอร์เสริม (ต่อท้าย URL ของ `/donate`)
| อยากทำ | ใส่ |
|---|---|
...
| ปรับขนาด/มุม/สี ของ Podium | ต่อท้าย URL หน้า Podium: `&podiumwidthpx=520` · `&podiumposition=bottom-right` · `&podiumscale=1.1` · `&tint1=sepia(.9) saturate(2.4)` |
| จูนควันของ Podium | `&smoke=A2&smokespreadx=1.1&smokespready=1&smokefit=1` (ชุดเดียวกับหน้า `/overlay`) |
| แผงวินิจฉัย Podium | `&podiumdiag=1` (โชว์ mode=live/all · donors · SSE state) |
