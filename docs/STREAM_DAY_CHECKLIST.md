...
| **ฟอนต์** | **ชุดเดียวกับการ์ด Last Follow เป๊ะ**: Montserrat (300/500/600 · self-host ด้วย `next/font` ที่ `src/app/podium/page.tsx`) + Noto Sans Thai เป็น fallback · น้ำหนัก 600 · letter-spacing/เงานุ่มคัดค่ามาจาก `.lt-label` / `.lt-name` / `.lt-timer` ของ `alert.html` |
...
| ตำแหน่ง/ขนาด | มุมเดิมของการ์ด Last Follow (`bottom-left`) · กว้าง ≈686px @1920 (ใหญ่กว่าการ์ด Last Follow ~1.8 เท่า) — ปักเองด้วย `?podiumwidthpx=` |
| ขนาดตัวอักษร | ชื่อ/ยอด `clamp(30px, 3.5vw, 58px)` · เลขอันดับ `clamp(22px, 2.6vw, 42px)` · หัวข้อ `clamp(13px, 1.35vw, 19px)` — ขยาย/ย่อทั้งชุด (รวมระยะห่าง) ด้วย `?podiumscale=1.2` |
