...
**ทดสอบโหมด “Edge เป็นตัวอ่านเสียง” (เปิด 2 หน้าต่างพร้อมกัน)**

1. เปิด OBS Browser Source ด้วย URL ที่มี **`&tts=0`** (ปิดการอ่านเสียงของ OBS — การ์ด/เสียง alert ยังทำงาน)
2. เปิด **Edge** ที่ `/overlay/voice?token=<OVERLAY_TOKEN>` → หน้าจะโชว์เสียงที่เลือก + ปุ่ม **▶ ทดสอบเสียง**
3. Admin → 🧪 ทดสอบ Alert → **การ์ดเด้งใน OBS** และ **ได้ยินเสียงผู้หญิงจาก Edge**
   (Edge จะเลือก `Microsoft เปรมวดี Online (Natural)` ให้เอง ส่วน OBS/CEF จะเห็นแค่ `Pattara` = ชาย)
4. ต้อง**ไม่มีเสียงอ่านซ้ำสองครั้ง** · คิวเดินปกติ (เฉพาะ OBS ที่ ACK) · กด Emergency TTS mute → Edge หยุดอ่านทันที
